...
import { Button, Loot, Tag, Gauge, Panel, RankDot, percent } from "@/components/ui";
...
export function ClusterCard({ cluster, targets, onOpen, onTarget, className }: ClusterCardProps) {
  ...
  return (
    <Panel className={className} as="article">
      ...
      {onOpen ? (
        <div className={styles.cluster__actions}>
          <Button ton="secondaire" onClick={onOpen} fullWidth>
            Open the cluster
          </Button>
        </div>
      ) : null}
